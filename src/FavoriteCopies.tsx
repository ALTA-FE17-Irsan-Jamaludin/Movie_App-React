...
import optionsFavorite from "./Data/favoriteMovies";
...
function FavoriteCopies() {
  ...
  const favoriteAPI = async () => {
    try {
      const response = await axios.request(optionsFavorite);
      setNilaiOutput((prevNilai) => ({ ...prevNilai, apiDataFavorite: response.data.results }));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const kirimAPI = async (item: Movie) => {
    const { title, original_title, rating, poster_path } = item;

    await axios
      .post(`https://658c0031859b3491d3f54070.mockapi.io/Movies`, { title, original_title, rating, poster_path })
      .then((response) => {
